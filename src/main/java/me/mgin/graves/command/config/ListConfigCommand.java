...
public class ListConfigCommand {
    ...
    private static void executeOnServer(CommandContext<ServerCommandSource> context, Responder res) {
        ...
        try {
            ...
            nbt.remove("palette");
            res.sendInfo(new TranslatableText("command.server.config.list", NbtHelper.toPrettyPrintedText(nbt)), null);
     ...
     * @param res Responder
     */
    private static void executeOnClient(CommandContext<ServerCommandSource> context, Responder res) throws CommandSyntaxException {
        ServerCommandSource source = context.getSource();
        ...
        if (player != null) {
            ...
            try {
                NbtCompound nbt = NbtHelper.fromNbtProviderString(config.serialize());
                nbt.remove("palette");
                res.sendInfo(new TranslatableText("command.config.list", NbtHelper.toPrettyPrintedText(nbt)), null);
            } catch (CommandSyntaxException e) {
                throw new RuntimeException(e);
            }
        } else {
