...
public class ResetConfigCommand {
    ...
    public static void executeOnServer(Responder res) {
        ...
        res.sendSuccess(new TranslatableText("command.server.config.reset:success"), null);
    }
     ...
     * @param res Responder
     */
    public static void executeOnClient(CommandContext<ServerCommandSource> context, Responder res) throws CommandSyntaxException {
        ServerCommandSource source = context.getSource();
        ServerPlayerEntity player = source.getPlayer();

        if (player != null) {
            ServerPlayNetworking.send(player, ConfigNetworking.RESET_CONFIG_S2C, PacketByteBufs.create());
            res.sendSuccess(new TranslatableText("command.config.reset:success"), null);
        } else {
