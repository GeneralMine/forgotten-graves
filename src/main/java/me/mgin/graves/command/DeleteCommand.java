...
import me.mgin.graves.util.Dimension;
...
public class DeleteCommand {
    static public int execute(CommandContext<ServerCommandSource> context) throws CommandSyntaxException {
        ...
        if (player == null) {
            res.sendError(new TranslatableText("command.generic.error.specify-player"), null);
        ...
        if (playerState.graves.size() >= graveId) {
            ...
            res.sendInfo(new TranslatableText("command.delete.deleted-grave", graveId, player.getName()), null);
            ...
            if (!grave.getBoolean("retrieved")) {
                ...
                if (destructive) {
                    ...
                    for (ServerWorld world : server.getWorlds()) {
                        String dimension = Dimension.getKey(world);
