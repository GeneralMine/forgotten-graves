...
public class ListCommand {
    static public int execute(CommandContext<ServerCommandSource> context) throws CommandSyntaxException {
        ...
        if (player == null && source.getPlayer() == null) {
            res.send(new TranslatableText("command.generic.error.specify-player"), null);
        ...
        if (source.getPlayer() != null && player != null) {
            ...
            if (!sourcePlayer.hasPermissionLevel(4) && !sourcePlayer.getUuid().equals(player.getId())) {
                res.sendError(new TranslatableText("command.generic.error.no-permission"), null);
                return Command.SINGLE_SUCCESS;
            }
        }
    ...
    public static void executeWithoutCommand(Responder res, GameProfile target, GameProfile recipient, int page,
        ...
        if (startOfPage >= config.server.storedGravesAmount || startOfPage >= graves.size()) {
            res.sendInfo(page == 1 ?
                    new TranslatableText("command.list.error.no-graves", res.highlight(target.getName())) :
                    new TranslatableText("command.list.error.no-graves-on-page", page, res.highlight(target.getName())),
                null);
            return;
        }
    ...
    private static void sendGraveList(Responder res, GameProfile target, ServerPlayerEntity issuer, GameProfile recipient, int page,
        ...
        // Seperator (no prefix)
        if (issuer != null) res.send(new LiteralText(""), null);

        // Page header
        res.sendInfo(new TranslatableText("command.list.header", res.highlight(target.getName())), null);
        ...
        for (Map.Entry<Integer, NbtCompound> entry : graves.entrySet()) {
            ...
            Text itemMessage = new LiteralText("") // Helps prevent passing styles to other Text objects
    ...
    private static Text genListCommandEntry(Responder res, NbtCompound grave, ServerPlayerEntity issuer,
        ...
        Text hoverContent = res.info(new TranslatableText(
        ...
        if (issuer == null) {
            message = new LiteralText("")
                ...
                .append(new TranslatableText("grave.coordinates", xText, yText, zText))
                .append(retrieved ? new LiteralText(" (✓)") : new LiteralText(""));
        } else {
            message = new TranslatableText("grave.coordinates", xText, yText, zText);
        }
        ...
        if (issuer != null && issuer.hasPermissionLevel(4)) {
            ...
            message = message.copy()
                ...
                .append(Interact.generateButton(res,
                    ...
                    res.hint(new TranslatableText("command.list.entry.restore-button.tooltip", i + 1, rec)),
            ...
            message = message.copy()
                ...
                .append(Interact.generateButton(res,
                    ...
                    res.hint(new TranslatableText("command.list.entry.delete-button.tooltip", i + 1)),
