...
public class ConfigSetter {
     ...
     */
    public void setConfig(CommandContextData data) throws CommandSyntaxException {
     ...
     * @param data CommandContextData
     */
    private void sendResponse(String slug, CommandContextData data) throws CommandSyntaxException {
        ...
        if (data.ACTION != null) {
            action = new TranslatableText(text + ":" + data.ACTION);
        }
        ...
        if (data.IS_SERVER) {
            onServer = new TranslatableText("command.config.set:success-server");
        }

        // Resolve response text
        Text response = switch (Objects.requireNonNull(slug)) {
            case "success" -> new TranslatableText(text, option, val, onServer);
            case "success-client-options" -> new TranslatableText(text, val, action, onServer);
            case "error.invalid-enum-value" -> new TranslatableText(text, option);
            case "error.invalid-config-option" -> new TranslatableText(text, val);
            case "error.nothing-changed-client-options" -> new TranslatableText(text, val, action);
            default -> new LiteralText(slug);
        };
