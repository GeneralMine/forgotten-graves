...
public class RestoreCommand {
    static public int execute(CommandContext<ServerCommandSource> context) throws CommandSyntaxException {
        ...
        if (graveId > playerState.graves.size()) {
            res.sendError(new TranslatableText("command.generic.error.grave-doesnt-exist", name), null);
            return Command.SINGLE_SUCCESS;
        }
        ...
        if (entity == null) {
            res.sendError(new TranslatableText("command.generic.error.player-not-online", entityProfile.getName()), null);
            return Command.SINGLE_SUCCESS;
        }
    ...
    public static void sendPlayerFeedback(ServerCommandSource source, GameProfile player, PlayerEntity entity,
                                          Responder res, Text hoverContent) throws CommandSyntaxException {
        ...
        if (player.equals(sourceProfile) && player.equals(entityProfile)) {
            res.sendInfo(res.hoverText(
                new TranslatableText("command.restore.grave-restored"),
                hoverContent
            ), null);
            return;
        }
        ...
        if (player.equals(entityProfile)) {
            res.sendInfo(res.hoverText(
                new TranslatableText("command.restore.grave-restored-by", source.getName()),
                hoverContent
            ), entity);
            ...
            if (sourceIsPlayerEntity) {
                res.sendInfo(res.hoverText(
                    new TranslatableText("command.restore.restored-players-grave", entityProfile.getName()),
                    hoverContent
                ), null);
            }
            return;
        ...
        if (!player.equals(entityProfile) && !entityProfile.equals(sourceProfile)) {
            ...
            res.sendInfo(res.hoverText(
                new TranslatableText("command.restore.received-players-grave-by", player.getName(), source.getName()),
                hoverContent
            ), entity);
            ...
            if (sourceIsPlayerEntity) {
                res.sendInfo(res.hoverText(
                    new TranslatableText("command.restore.restored-players-grave-to", player.getName(), entityProfile.getName()),
                    hoverContent
                ), null);
            }
        ...
        if (entityProfile.equals(sourceProfile) && !entityProfile.equals(player)) {
            res.sendInfo(res.hoverText(
                new TranslatableText("command.restore.received-players-grave", player.getName()),
                hoverContent
            ), entity);
        }
    }
    ...
    public static Text generateHoverContent(Responder res, NbtCompound grave, int graveId, String source) {
        ...
        return res.info(new TranslatableText(
