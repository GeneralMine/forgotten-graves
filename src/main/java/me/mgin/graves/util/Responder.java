...
public class Responder {
    ...
    public void send(Text message, PlayerEntity target) {
        ...
        if (recipient != null) recipient.sendMessage(response, false);
        else server.sendSystemMessage(response, null);
    }
    ...
    public void sendError(Text message, PlayerEntity target) {
        send(ResponderTheme.style(new TranslatableText("error.prefix").append(message), "error"), target);
    }
    ...
    /**
     * Converts passed value into a new LiteralText.
     *
     * @param value Object
     * @return Text
     */
    private Text genValueText(Object value) {
        return value instanceof Text ? (Text) value : new LiteralText(String.valueOf(value));
    }
